...
x <- runif(m,0,2)
phix = 2*dnorm(x)
theta.hat <- mean(phix)+1/2
print(theta.hat)
print(pnorm(2))

sd.mc = sd(phix)/sqrt(m)

theta.hat +1.96*sd.mc*c(-1,1)
...
curve(g,from=0,to=1.5,col='black',ylim=c(0,2),ylab = 'function values')
curve(f0,from=0,to=1.5,add = T,lty=2,lwd=2,col='red')
curve(f1,from=0,to=1.5,add = T,lty=3,lwd=3,col='blue')
curve(f2,from=0,to=1.5,add = T,lty=4,lwd=4,col='green')
curve(f3,from=0,to=1.5,add = T,lty=5,lwd=5,col='brown')
curve(f4,from=0,to=1.5,add = T,lty=6,lwd=6,col='purple')
legend(x='topright',legend = c('g','f0','f1','f2','f3','f4'),col = c('black','red','blue','green','brown','purple'),lty=1:6,lwd = 1:6)


#' Even though $f_1$ is the closest to $g$ in values, its range is from 0 to infinity, so the ratio of $g(x)$ and $f_1(x)$ is far from being constant.
#' The function that corresponds to the most nearly constant ratio $g(x)/f(x)$ appears to be $f_3$.
#'
#' Let us also plot the ratios of $g$ and each potential importance function.
rgf0 <- function(x) g(x)/f0(x)
rgf1 <- function(x) g(x)/f1(x)
rgf2 <- function(x) g(x)/f2(x)
rgf3 <- function(x) g(x)/f3(x)
rgf4 <- function(x) g(x)/f4(x)

curve(rgf0,from=0,to=1.5,add = F,lty=2,lwd=2,col='red')
curve(rgf1,from=0,to=1.5,add = T,lty=3,lwd=3,col='blue')
curve(rgf2,from=0,to=1.5,add = T,lty=4,lwd=4,col='green')
curve(rgf3,from=0,to=1.5,add = T,lty=5,lwd=5,col='brown')
curve(rgf4,from=0,to=1.5,add = T,lty=6,lwd=6,col='purple')
#legend(x='topright',legend = c('rgf0','rgf1','rgf2','rgf3','rgf4'),col = c('red','blue','green','brown','purple'),lty=2:6,lwd = 2:6)
...
#'
#' I decide to add another example.
#'
#'  Consider MC estimate of $\int_1^\infty x^{-2}dx$. As $x\rightarrow \infty$, $x^{-2}$ behave similarly to $\frac{1}{1+x^2}$. So maybe Cauchy distribution can be used as the importance function. The support of Cauchy is the whole real line, which is much wider than $[1,\infty)$.However, a simple linear transformation can create a random variable on $[1,\infty)$, and its pdf behave like Cauchy. Define $X=1+|Z|$ where $Z$ follows a Cauchy distribution, you can find the pdf of $X$ is $f(x) = \frac{2}{\pi[1+(x-1)^2]}$.
#'Another possible choice is shifted exponential

g = function(x) ifelse(x>=1,x^(-2),0)
f = function(x) ifelse(x>=1,2/(pi*(1+(x-1)^2)),0)
f2 = function(x) ifelse(x>=1,exp(-1*(x-1)),0)
curve(g,0,10)
curve(f,0,10,add=T,lty=2,lwd=1)
curve(f2,0,10,add=T,lty=3,lwd=2)
m=100

# simple transformation to sample from a distribution with pdf f
xc = rcauchy(m)
xn =1+abs(xc)

# estimate with importance sampling
gf <- g(xn) / f(xn)
mean(gf)
sd(gf)/sqrt(m)
...
g = function(x) ifelse(x>=1&x<5,x^(-2),0)
f = function(x) ifelse(x>=1,2/(pi*(1+(x-1)^2)),0)
f2 = function(x) ifelse(x>=1,exp(-1*(x-1)),0)
curve(g,0,10)
curve(f,0,10,add=T,lty=2)
curve(f2,0,10,add=T,lty=3)
m=100

# simple transformation to sample from a distribution with pdf f2
xc = rcauchy(m)
xn =1+abs(xc)
...
#'
g = function(x) ifelse(x>0&x<1,sin(pi*x),0)

x.grid = 0.01*(0:100)
plot(x.grid, g(x.grid),type='l',ylim=c(0,3))
for(ii in 1:10) lines(x.grid,dbeta(x.grid,shape1 = 0.8+0.2*ii,shape2 = 0.8+0.2*ii),col=ii,lty=ii,lwd=ii)

estSinByBeta = function(m,shape){
x = rbeta(m,shape,shape)
gd = sin(pi*x)/dbeta(x,shape,shape)
est = mean(gd)
se = sd(gd)/sqrt(m)
output = c(est,se)
...
#'  we estimate each of $\int_0^{0.25} \frac{e^{-x}}{1+x^2} dx$, $\int_{0.25}^{0.5} \frac{e^{-x}}{1+x^2} dx$, $\int_{0.5}^{0.75} \frac{e^{-x}}{1+x^2} dx$ and $\int_{0.75}^1 \frac{e^{-x}}{1+x^2} dx$ using $M/4=2500$ random samples from the corresponding uniform distribution over its interval.
#'
...
estStrata <- function(x,k){ # this function is for the stratified sampling estimator of the integral with k strata
  estGJ = function(j) (x/k)*mean(dnorm(runif(m/k,x*(j-1)/k,x*j/k))) # For given j, this internal function estimate the integral over interval [(j-1)/k,j/k]
  est.strata.vec = sapply(1:k, estGJ)
  return(sum(est.strata.vec)+1/2)
}
