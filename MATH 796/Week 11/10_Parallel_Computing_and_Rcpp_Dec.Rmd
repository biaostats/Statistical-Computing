...
In a practice problem on Bootstrap, we computed the bootstrap based CIs for the correlation between Assault rate and urbanPop in USArest data. Recall that for Percentile or Basic CIs, we need a large number of Bootstrap replicates, and it can be computationally intensive. Now let us try to speed it up using Rcpp. Since the computationally intensive part is the bootstrapping process but not how to calculate the intervals with bootstrap output, we will focus on the bootstrapping part.
...
library(Rcpp)
library(RcppArmadillo)
sourceCpp('10_Parallel_Computing_and_Rcpp_Dec.cpp')
ls()
