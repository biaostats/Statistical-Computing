...
#'
f.prac = function(x) x*(x-1)*(x-2)*(x-4)
f.grad.prac = function(x) (x-1)*(x-2)*(x-4)+ x*(x-2)*(x-4)+ x*(x-1)*(x-4)+x*(x-1)*(x-2)
...
x.all = x.init
while(err.x>eps&iter<iter.max){
  x.new = update.gd.bb(xk=x.init,xk1=x.init1, f.grad=f.grad.prac)
  err.x = sqrt(sum((x.new-x.init)^2))
  x.all = rbind(x.all,x.new)
  iter=iter+1
  x.init1=x.init
...
f.hess.prac <- function(x) (x-2)*(x-4)+ (x-2)*(x-4)+ (x-1)*(x-4)+(x-1)*(x-2)+(x-1)*(x-4)+ x*(x-4)+ x*(x-4)+x*(x-2)+(x-1)*(x-2)+ x*(x-2)+ x*(x-1)+x*(x-1)
...
x.all = x.init
while(err.x>eps&iter<iter.max){
  x.new = update.newton(xk=x.init, f.grad=f.grad.prac,f.hess=f.hess.prac)
  err.x = sqrt(sum((x.new-x.init)^2))
  x.all = rbind(x.all,x.new)
  iter=iter+1
  x.init=x.new
...
set.seed(1201)
x = 1:10
y = 1+2*x+3*rnorm(length(x))
plot(x,y)

f.lse = function(beta,x,y) sum((y-beta[1]-beta[2]*x)^2)

f.grad.lse = function(beta,x,y) c(-2*sum(y-beta[1]-beta[2]*x),-2*sum(x*(y-beta[1]-beta[2]*x)))

f.hess.lse = function(beta,x,y) matrix(c(2*length(x),2*sum(x),2*sum(x),2*sum(x^2)),2,2)
...
#' Report the estimates at the original scale.
if(iter==iter.max)  print(paste0('The algorithm does not converge in ',iter.max,' iterations!'))
if(err.x<eps) print(paste0('The algorithm converges at (alpha,beta)= (',paste0(beta.new,collapse = ','),') at iteration ',iter,'.'))
...
#' #### R function optim
#'
#' optim is one of the most widely used general purpose optimizer in R that offers a wide range of algorithm choices. I usually use the gradient-free algorithm "Nelder-Mead", or "L-BFGS-B" which is a Quasi-Newton approach. I cannot find the official website of all the algorithms implemented by optim (I used to know it five years ago). But most of the algorithms on this website are implemented (https://nlopt.readthedocs.io/en/latest/NLopt_Algorithms/).
#'
...
#' #' It also comes to my attention that many argues that R function optim is out of date due to issues in coding, and one should use some newer packages for general optimization (e.g., \url{https://www.r-bloggers.com/2016/11/why-optim-is-out-of-date/}).
#' While they are making good points, I have not tested newer packages in practice. Based on my readings, strong contenders include nloptr and optimx. The algorithm I am also very excited about CVXR, a package for convex optimization.
#'
#'
#'
#' #### Practice
...
x = 1:10
y = 1+2*x+3*rnorm(length(x))
out.lse = optim(par=c(0,0),fn=function(beta) f.lse(beta,x,y),method =  "L-BFGS-B")
out.lse$par
lm(y~x)
