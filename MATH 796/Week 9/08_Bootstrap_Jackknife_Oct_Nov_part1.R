...
B = 100
set.seed(20211101)
boot.cor.density = replicate(B,boot.cor.density(USArrests))

hist(boot.cor.density,breaks = 20)

(se.boot.cor.density = sd(boot.cor.density))
(bias.boot.cor.density = mean(boot.cor.density)-cor(USArrests$Assault,USArrests$UrbanPop))
bias.boot.cor.density/se.boot.cor.density
#'
...
#' \[
#' \hat{bias}_{jack} = (n-1)(\bar{\hat{\theta}_{(\cdot)}})
#' \]
#' and
#' \[
...
#'
#' The jackknife can fail when the estimator $\hat{\theta}$ is not a "smooth" function of the data.
...
#'
jack.cor.density <- function(i,dt){
  boot.samp.b = dt[-i,]
  boot.rep.b = cor(boot.samp.b$Assault,boot.samp.b$UrbanPop)
  return(boot.rep.b)
}
...
n = nrow(USArrests)
set.seed(20211101)
jack.cor.density = sapply(1:n,jack.cor.density, dt = USArrests)

hist(jack.cor.density,breaks = 20)

(se.jack.cor.density = (n-1)/sqrt(n)*sd(jack.cor.density))
(bias.jack.cor.density = (n-1)*(mean(jack.cor.density)-cor(USArrests$Assault,USArrests$UrbanPop)))
bias.jack.cor.density/se.jack.cor.density
#'
