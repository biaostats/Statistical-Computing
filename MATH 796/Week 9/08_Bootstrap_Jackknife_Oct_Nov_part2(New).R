...
(cor.density = cor(USArrests$Assault, USArrests$UrbanPop))

boot.cor.density <- function(dt){
  n = nrow(dt)
  id.b = sample(1:n,size=n,replace = T)
  boot.samp.b = dt[id.b,]
...
B = 999
set.seed(2046)
arrest.boot.cor.density = replicate(B,boot.cor.density(USArrests))

p.upper = 1-0.1/2
p.lower=0.1/2

#' Standard normal bootstrap CI is just a z interval with se estimated using bootstrap. If using this method, you could have just set $B=100$
(se.arrest.boot.cor.density = sd(arrest.boot.cor.density))
(lb.arrestcordensity.normal = cor.density-qnorm(p.upper)*se.arrest.boot.cor.density)
(ub.arrestcordensity.normal = cor.density+qnorm(p.upper)*se.arrest.boot.cor.density)


#' Percentile bootstrap CI just use the percentiles of the bootstrap replicates.
(lb.arrestcordensity.percentile = quantile(arrest.boot.cor.density,probs = p.lower,names = F))
(ub.arrestcordensity.percentile = quantile(arrest.boot.cor.density,probs = p.upper,names = F))

#' Basic bootstrap CI uses the percentiles of the distribution of the bootstrap relicates after adjusting for $\hat{\theta}$.
(lb.arrestcordensity.basic = 2*cor.density-quantile(arrest.boot.cor.density,probs = p.upper,names = F))
(ub.arrestcordensity.basic = 2*cor.density-quantile(arrest.boot.cor.density,probs = p.lower,names = F))
...
library(boot)
boot.cor.density <- function(dt, i){
  boot.samp.b = dt[i,]
  boot.rep.b = cor(boot.samp.b$Assault, boot.samp.b$UrbanPop)
  return(boot.rep.b)
}
boot.cor = boot(USArrests, boot.cor.density, R = 999, stype = "i")
boot.ci(boot.cor, type = c('norm','basic','perc'), conf = .9)
