...
###' Split state.df$Income by state.df$Region, and call the output income.by.reg. 
...
tapply(state.df$Murder, INDEX = substr(rownames(state.df),1,1), mean)
###' 
...
system.time(log.vec.for <- for.log(c(1:n)))
###' b) For loop
log.vec.for.2 = rep(NA, n)
system.time(for (i in 1:n) log.vec.for.2[i] <- log(i))
###' c) sapply
system.time(log.vec.sapply <- sapply(c(1:n), log))
###' 
###' 5. If you substitute "<-" in system.time(log.vec.vec<- log(1:n)) with "=", what will you get? Can you guess why?
###' Answer:
###' system.time(log.vec.vec= log(1:n)) returns an error.The operators <- and = assign into the environment in which 
###' they are evaluated. The operator <- can be used anywhere, whereas the operator = is only allowed at the top level 
###' (e.g., in the complete expression typed at the command prompt) or as one of the subexpressions in a braced list 
###' of expressions.
