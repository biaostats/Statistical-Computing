...
par(mfrow = c(2,1), mar = c(4, 4, 2, .5))
plot(x.0, y.0, col = 'green', pch = 19, cex = 0.5, main = "Shrunken green filled circle")
plot(x.0, y.0, col = 'purple', pch = 21, cex = 3, main = "Expanded purple empty circles")
###'
...
par(mfrow = c(1,1))
plot(x.0, y.0, xlim = c(-1,1), ylim = c(-5,5), xlab = "Trimmed x", ylab = "Trimmed y")
###'
...
plot(x.0, y.0, pch = c(19,21), col = c('black','blue','black','red'))
###' 5. Produce a scatter plot of `y` versus `x` (as in the lecture), and set the title and axes labels as you see fit.
plot(x.0, y.0, main = "The plot of y versus x", xlab = "x", ylab = "y") 
...
x.2 = sort(runif(n, min=-2, max=2))
y.2 = x.2^2 + rnorm(n)
points(x.2, y.2, pch = 19, col = 'blue')
...
legend(x = 1.225, y = -1.08, legend = c("Cubic", "Quadratic"), col = c('red', 'blue'), lwd = 1, lty = c(1,2), pch = c(NA, NA), cex = .8, text.col = 'white')
legend(x = 1.225, y = -1.05, legend = c("Cubic", "Quadratic"), col = c('black', 'blue'), lwd = 1, lty = c(0,0), pch = c(21, 19), cex = .8, text.col = 'black', bty = 'n')
...
for (i in c(1:ncol(insurance.df))) {
  if (class(insurance.df[,i]) == "character"){
      barplot(table(insurance.df[,i]), xlab = colnames(insurance.df)[i],
              main = paste("Barplot of", colnames(insurance.df)[i]))
  } else {
    hist(insurance.df[,i], xlab = colnames(insurance.df)[i],
         main = paste("Histogram of", colnames(insurance.df)[i]))
  }
}
par(mfrow = c(1,1))
...
par(mfrow = c(2,4), mar = c(4,4,2,.5))
for (i in which(colnames(insurance.df) != "charges")) {
  if (class(insurance.df[,i]) == "character"){
    boxplot(insurance.df$charges~factor(insurance.df[,i]), xlab = colnames(insurance.df)[i],
            ylab = "charges", main = paste("Charges vs", colnames(insurance.df)[i]))
  } else {
    plot(insurance.df$charges, insurance.df[,i], xlab = colnames(insurance.df)[i],
         ylab = "charges", main = paste("Charges vs", colnames(insurance.df)[i]))
  }
}
par(mfrow = c(1,1))
...
insurance.cor = cor(insurance.df[,which(sapply(insurance.df, class) != "character")])
insurance.cor[,"charges"]
insurance.df$LogCharges = log(insurance.df$charges)
insurance.log.cor = cor(insurance.df[,which(sapply(insurance.df, class) != "character" & colnames(insurance.df) != "charges")])
insurance.log.cor[,"LogCharges"]
insurance.df$LogCharges = NULL
rm(insurance.log.cor)
...
insurance.pred = data.frame(age = 28, sex = "male", bmi = 30, children = 0, smoker = "yes", region = "southeast")
exp(predict(insurance.lm, newdata = insurance.pred))
